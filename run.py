...
def Prediction_Attention():
    files = sorted(os.listdir("processed_data/"))
    for file in files:
        full_path = os.path.join("processed_data/", file)
        base_name = os.path.splitext(file)[0]
        file_size = os.path.getsize(full_path)

        if file_size < 500 * 1024 * 1024:  # File size less than 500MB
            print(f"{current_time()} - start direct prediction {file}")
            output_filename = f"results/probs_{base_name}.txt"
        ...
        os.remove(full_path)  # Delete the original processed_data file
        os.remove(f"Data/{base_name}.fa")  # Delete Data/{file}.fa if needed
        os.remove(f"results/probs_{base_name}.txt")  # Delete intermediate results file
